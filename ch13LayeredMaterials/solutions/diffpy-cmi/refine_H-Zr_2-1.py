...
data_file = "../../data/pdfs/H-Zr_2-1_PP_cutFirstPeak.gr"
stru_file = Structure(filename="../../data/structures/alpha_ZrP_enlargedZ.cif")
basename = "H-Zr_2-1"
...
def main():
    ...
    # 'psize' because we want to refine the crystallite size.
    recipe.fix("all")
    tags = ["lat", "scale", "adp", "d1", "psize", "all"]
    for tag in tags:
    	recipe.free(tag)
        least_squares(recipe.residual, recipe.values, x_scale="jac")

    # Save structures
    stru_file.write(basename + ".cif", "cif")
